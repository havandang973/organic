                    ...
                    <form class="updateCartForm" action="{{route('update')}}" method="POST">
                                        ...
                                        <th scope="row">
                                            <div class="thamnail_img">
                                                <a href="{{route('productDetail', ['id' => $row->id])}}" class="">
                                                    <img class="img-fluid" width="155" src="{{$row->options->thumbnail}}" alt="" />
                                                </a>
                                            </div>
                                        </th>
                                        <td class="align-middle"><b>{{$row->name}}</b></td>
                                        <td class="align-middle font-weight-bold">{{number_format($row->price)}}₫</td>

                                        <td class="align-middle">
                                            <div class="cart d-flex">
                                                <div class="d-flex">
                                                    <input name="qty[{{$row->rowId}}]" id="" type="number" class="qty w-50 text-center font-weight-bold" placeholder="1" min="1" value="{{$row->qty}}" style="outline: none">
                                                </div>
                                            </div>
                                        </td>
                                        <td class="align-middle font-weight-bold">{{number_format($row->qty * $row->price)}}₫</td>
                                        <td class="align-middle"><i class="icofont-close text-danger"></i></td>
                                        <td class="text-center" style="cursor: pointer;">
                                            <a data-row-id="{{$row->rowId}}" class="removeCart text-danger"><i class="fa-solid fa-xmark" style="font-size: 20px"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                        ...
                        <button type="button" class="updateCart btn btn-success float-right">Cập nhật</button>
                            <div id="error-item" class="d-none items-center p-4 mb-4 text-sm text-danger" role="alert">
                                <div class="d-flex">
                                    <svg class="flex-shrink-0 inline w-4 h-4 me-3" width="16" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div class="order_cardTotal">
                        <table class="table">
                            ...
                            <tr>
                                ...
                                <td class="total text-right total_num" style="color: #54b215">{{Cart::total()}} đ</td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
